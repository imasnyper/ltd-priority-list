...
from scheduler.periods import weekday_abbrs, weekday_names
...
@register.inclusion_tag('scheduler/_month_table.html', takes_context=True)
...
@register.inclusion_tag('scheduler/_day_cell.html', takes_context=True)
...
@register.inclusion_tag('scheduler/_daily_table.html', takes_context=True)
...
@register.inclusion_tag('scheduler/_event_title.html', takes_context=True)
...
@register.inclusion_tag('scheduler/_event_options.html', takes_context=True)
...
@register.inclusion_tag('scheduler/_create_event_options.html', takes_context=True)
...
class CalendarNode(template.Node):
    ...
    def render(self, context):
        ...
        context[self.context_var] = Calendar.objects.get_calendar_for_object(self.content_object.resolve(context),
                                                                             self.distinction)
        return ''
...
def get_calendar(parser, token):
    ...
    if len(contents) == 4:
        ...
        raise template.TemplateSyntaxError("%r tag follows form %r <content_object> as <context_var>" % (
